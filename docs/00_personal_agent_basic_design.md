...
| F-05 | 日次蒸留 | P2 | セッションログ → 前処理 → LLM → 日次レポート |
| F-08 | 週次・月次蒸留 | P2 | 日次 → 週次 → 月次の段階的集約 |
| F-06 | MEMORY.md 自動更新 | P2 | 蒸留結果を「直近セクション」に反映 |
...
蒸留パイプライン（P2）や Web コンソール（P3）で完全な会話履歴が必要なため、
...
用途:
- **P2 蒸留パイプライン**: `logs/*/*.jsonl` を日付で集めて前処理 → LLM → 日次レポート
...
独自会話ログ (F-14)  →  蒸留・Web コンソール・振り返りのため（永続的）
                        PicoClaw の要約に影響されない
...
### Phase 2: 蒸留パイプライン — 「記憶の整理」

**ゴール**: セッションログを段階的に蒸留し、長期メモリを構築する。
...
| 蒸留レポート | `memory/daily/`, `memory/weekly/` | ファイル読み取り |
...
- 外部プロセス (蒸留パイプライン, Web コンソール, MCP サーバー)
...
| # | 項目 | 関連 Phase | メモ |
|---|------|-----------|------|
| TBD-01 | 蒸留パイプラインの実装言語 | P2 | Go 統一 vs Python。テキスト処理の書きやすさと依存管理のトレードオフ |
| TBD-02 | Web コンソールの技術スタック | P3 | フロントエンド技術、サーバー側の言語 |
| TBD-03 | MCP サーバーの実装言語 | P2 | 蒸留パイプラインと同一言語が望ましい |
| ~~TBD-04~~ | ~~蒸留の実行方式~~ | P2 | **解決済み**: picapica-nest 内蔵スケジューラでトリガー。LLM は Claude Code CLI (`--print`) 経由で利用 |
